...
for portion_idx in range(NUM_PORTIONS):
    ...
    max_amplitude_over_time.append(np.abs(fft_result[max_amplitude_index]))
    


    # Check if the last four values are the same
    if (portion_idx > 4):
        if (max_frequency_over_time[-1] == max_frequency_over_time[-2] == max_frequency_over_time[-3] == max_frequency_over_time[-4] and max_frequency_over_time[-1] > 20 and max_amplitude_over_time[-1] > 5000):
...
for i in range(len(valid_notes)):
    if ((i + 1) % 10 == 1 and i+1 != 11):
        print("{}st highest note played: {} at frequency {}".format(i + 1, valid_notes[i], frequencies_played[i]))
    elif ((i + 1) % 10 == 2 and i+1 != 12):
        print("{}nd highest note played: {} at frequency {}".format(i + 1, valid_notes[i], frequencies_played[i]))
    elif ((i + 1) % 10 == 3 and i+1 != 13):
        print("{}rd highest note played: {} at frequency {}".format(i + 1, valid_notes[i], frequencies_played[i]))
    else:
        print("{}th highest note played: {} at frequency {}".format(i + 1, valid_notes[i], frequencies_played[i]))